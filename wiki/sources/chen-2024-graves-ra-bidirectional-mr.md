---
title: "Graves' Disease and Rheumatoid Arthritis: A Bidirectional Mendelian Randomization Study"
type: source
created: 2026-04-09
updated: 2026-04-09
authors: [Chen W, Liu Q, Chen Y, Zhou X, Zhang H, Li T]
journal: Frontiers in Endocrinology
year: 2024
doi: "10.3389/fendo.2023.1283698"
tags: [graves-disease, thyroid, autoimmune, rheumatoid-arthritis, mendelian-randomization, GWAS, HLA, comorbidity, JAK-STAT, immune-dysregulation]
platform: wikibiome
evidence_level: computational-prediction
metals_discussed: [copper, iron, selenium, zinc]
---

# Graves' Disease and Rheumatoid Arthritis: A Bidirectional Mendelian Randomization Study

## Key Claims

- Bidirectional two-sample Mendelian randomization reveals a significant causal relationship between [[graves-disease]] (GD) and [[rheumatoid-arthritis]] (RA), with genetically predicted GD increasing RA risk
- Shared autoimmune pathways involving [[autoimmunity|HLA]] alleles, particularly HLA-DRB1, and [[inflammation|JAK/STAT pathway]] signaling underlie the comorbidity between GD and RA
- The [[autoimmunity|PTPN22]] R620W variant is a shared susceptibility locus for both GD and RA, implicating T-cell receptor signaling dysregulation
- [[inflammation]] driven by TNF-alpha, IL-6, and other pro-inflammatory cytokines represents a common mechanistic link between the two conditions
- Shared environmental risk factors including smoking, [[selenium]] deficiency, and vitamin D insufficiency may exacerbate both conditions

## Methods/Findings

- **Study type**: Bidirectional two-sample Mendelian randomization using GWAS summary statistics
- **Methods**: Inverse variance weighted (IVW) as primary analysis; MR-Egger, weighted median, and MR-PRESSO for sensitivity analyses; Cochran's Q test for heterogeneity
- **Key genetic overlap**: HLA-DRB1 shared alleles confer risk for both GD and RA; PTPN22, CTLA-4, and STAT4 identified as shared susceptibility loci
- **Biological plausibility**: Both diseases involve loss of immune tolerance, with autoreactive T and B cells driving tissue-specific damage (thyroid in GD, synovium in RA)
- **Cytokine overlap**: Elevated TNF-alpha, IL-6, IL-17, and IFN-gamma observed in both conditions; [[nf-kappa-b]] pathway activation is central to both diseases
- **Clinical implications**: Patients with GD should be monitored for RA symptoms and vice versa; shared therapeutic targets (e.g., JAK inhibitors, anti-TNF) may benefit both conditions

## Connections

- Complements [[xian-2023-graves-disease-ibd-mendelian-randomization]] on GD comorbidity with other autoimmune diseases using similar MR methodology
- PTPN22 shared susceptibility connects to broader patterns across thyroid and joint diseases
- JAK/STAT pathway overlap relevant to as shared therapeutic targets, paralleling IBD-GD findings
- [[selenium]] deficiency as shared risk factor connects to [[brock-2015-selenium-thyroid-autoimmunity]] and role of [[oxidative-stress]] in both conditions
- [[nf-kappa-b]] activation as shared inflammatory driver relevant to understanding systemic autoimmune [[inflammation]]
- HLA-DRB1 shared susceptibility extends the pattern seen in [[xian-2023-graves-disease-ibd-mendelian-randomization]] with IBD
- Micronutrient connections via [[iron]], [[zinc]], and [[copper]] status in both autoimmune thyroid and joint diseases link to [[brylinski-2025-trace-elements-thyroid-diseases]]
