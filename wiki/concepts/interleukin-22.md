---
title: Interleukin-22
type: concept
created: 2026-09-07
updated: 2026-09-07
last_substantive_update: 2026-09-07
sources:
  - paeslack-2022-tryptophan-metabolites-vascular-inflammation-cvd
  - carretta-2021-scfas-receptors-gut-inflammation-colon-cancer
  - kong-2021-ketogenic-diet-colitis-ilc3-microbiome
  - ding-2025-mycobiome-human-cancer-mechanisms-therapeutics
  - thirion-2023-gut-microbiota-ms-disease-activity
  - cao-2021-dysbiotic-gut-microbiota-cytokine-profile-asd
  - ahlstrom-2019-nickel-allergy-review
source_count: 7
tags: [IL-22, cytokine, ILC3, Th17, mucosal-immunity, epithelial-barrier, antimicrobial-defense, AhR, short-chain-fatty-acids, mycobiome]
karen_brain_primitives: [1, 3, 4, 5]
seo_target: "interleukin-22 IL-22 microbiome gut barrier"
platform: wikibiome
---

# Interleukin-22

Interleukin-22 (IL-22) is an immune signaling protein that helps coordinate host responses at epithelial surfaces. In the gut evidence represented by WikiBiome, group 3 innate lymphoid cells (ILC3s) are a prominent source, while Th17-associated responses can also include IL-22. Its target context is the tissue barrier: IL-22 can support epithelial integrity, antimicrobial defense, and repair rather than acting only as a general marker of systemic [[inflammation]] [[carretta-2021-scfas-receptors-gut-inflammation-colon-cancer]] [[ahlstrom-2019-nickel-allergy-review]].

IL-22 is not uniformly beneficial or harmful. Its meaning depends on the producing cell, tissue, initiating signal, duration, disease state, and accompanying cytokines. Barrier-supporting activity in one model can coexist with inflammatory or tumor-associated activity in another.

## Microbiome-to-barrier signaling

One route begins with microbial [[tryptophan-metabolism]]. Indole metabolites can activate the [[ahr|aryl hydrocarbon receptor]], which regulates ILC3 activity and promotes IL-22 production in mucosal immunity. This provides a mechanistic bridge from microbial metabolism to a host epithelial response, but the direction and magnitude depend on the specific ligand and cell type [[paeslack-2022-tryptophan-metabolites-vascular-inflammation-cvd]].

A second route begins with [[short-chain-fatty-acids]]. A review of SCFA receptors describes FFAR2 signaling in ILC3s and places IL-22 downstream of this immune-cell program, where it supports tissue integrity and host defense. The same review emphasizes that SCFA receptors are expressed across epithelial and immune-cell populations, so an observed SCFA association should not automatically be assigned to IL-22 without pathway-level measurements [[carretta-2021-scfas-receptors-gut-inflammation-colon-cancer]].

These pathways explain why loss of indole-producing or SCFA-producing organisms can be discussed as loss of IL-22-supporting ecological function. They do not establish that the abundance of any single organism determines a person's IL-22 level.

## Barrier defense and tissue repair

Within homeostatic mucosal signaling, IL-22 is linked to epithelial repair and antimicrobial defense. It is often discussed alongside tight-junction integrity, mucin, IgA, and antimicrobial-peptide programs. These outcomes are related but are not interchangeable measurements: detecting IL-22 does not prove that the barrier has recovered, and improved barrier markers do not prove that IL-22 caused the improvement [[carretta-2021-scfas-receptors-gut-inflammation-colon-cancer]].

The producing-versus-responding-cell distinction is important. The evidence summarized here places IL-22 production in immune populations such as ILC3 or Th17-associated cells; epithelial cells are represented primarily as the responding tissue. Claims that colonocytes themselves produce IL-22 require separate direct evidence.

## Context-dependent inflammatory effects

An animal study of DSS colitis illustrates the context problem. A ketogenic diet altered the microbiome, reduced colonic ILC3 abundance and IL-22 expression, improved barrier markers, and reduced disease severity; fecal transfer reproduced part of the protective effect in germ-free mice [[kong-2021-ketogenic-diet-colitis-ilc3-microbiome]]. In that model, lower IL-22 accompanied improvement even though IL-22 can support barrier integrity in other settings. The study therefore argues against labeling every increase as protective or every decrease as harmful.

Outside the gut, a clinical review of nickel allergic contact dermatitis places IL-22 within a Th1/Th17-skewed T-cell response that also includes IFN-gamma and IL-17. This is a skin hypersensitivity context and should not be generalized to intestinal homeostasis [[ahlstrom-2019-nickel-allergy-review]].

## Fungal and tumor contexts

Fungal signals add another layer. A cancer-mycobiome review describes *Candida*- and other fungus-associated IL-22/ILC pathways within immunosuppressive and tumor-promoting microenvironments. It also reports poorer survival in a Candida-dominant gastrointestinal-cancer pattern associated with IL-22 and several tumor pathways [[ding-2025-mycobiome-human-cancer-mechanisms-therapeutics]]. These findings support an IL-22 connection in cancer ecology but do not show that IL-22 alone caused the survival difference.

This is why “barrier-protective cytokine” is an incomplete definition. IL-22 can participate in epithelial defense while also appearing in chronic inflammatory or tumor-associated networks. A page or study must specify tissue, cell source, disease state, and measured outcome before assigning direction.

## Human association and negative evidence

In a prospective case-control study of multiple sclerosis, disease-associated microbial species correlated with plasma IL-22 and other inflammatory cytokines. Such correlations connect microbiome composition to an immune profile, but they do not establish which organisms produced the signal or whether IL-22 drove disease activity [[thirion-2023-gut-microbiota-ms-disease-activity]].

Negative findings are equally important. A case-control study of children and adolescents with autism reported several cytokine differences, but IL-22 was not significantly different between groups [[cao-2021-dysbiotic-gut-microbiota-cytokine-profile-asd]]. WikiBiome treats that result as outcome-specific negative evidence rather than omitting it or interpreting it as proof that IL-22 is irrelevant to every autism subgroup.

## Evidence interpretation

- IL-22 concentration is a context-specific measurement, not a universal score of barrier health or inflammation.
- ILC3 and Th17-associated cells can produce IL-22; epithelial tissue is a major response compartment in the evidence summarized here.
- Microbial indoles and SCFAs provide plausible upstream routes, but organism abundance alone does not prove IL-22 pathway activity [[paeslack-2022-tryptophan-metabolites-vascular-inflammation-cvd]] [[carretta-2021-scfas-receptors-gut-inflammation-colon-cancer]].
- Animal diet, colitis, or fecal-transfer experiments establish biological plausibility; they do not prove a human dietary treatment effect [[kong-2021-ketogenic-diet-colitis-ilc3-microbiome]].
- Tumor, skin-allergy, neurologic, and intestinal findings should not be merged into a single beneficial-or-harmful label.
- A null result in one cohort and specimen type should remain visible alongside positive associations from other settings [[cao-2021-dysbiotic-gut-microbiota-cytokine-profile-asd]].

## Related pages

- [[ahr]]
- [[indoles]]
- [[tryptophan-metabolism]]
- [[short-chain-fatty-acids]]
- [[th17-treg-balance]]
- [[immune-balance]]
- [[tight-junctions]]
- [[intestinal-permeability]]
- [[mycobiome]]
- inflammation
- [[il-17]]
- [[il-10]]
