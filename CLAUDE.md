...
### Rule 11 — Claim-level citation density (Wikipedia parity)

**Rule:** Every material claim on a page marked `platform: wikibiome` or `platform: both` must have at least one inline `[[source-filename]]` reference at or near the claim. A claim is material if it asserts a mechanism, a number, an association, or a causal relationship. Narrative connective tissue does not require a citation; factual assertions do. The lint cycle counts citations per publishable paragraph; pages averaging fewer than one citation per three paragraphs are auto-demoted to `stub: true` with `stub_reason: "claim-level citation deficit — N citations across M paragraphs"` and logged to `wiki/analyses/citation-deficit-YYYY-MM-DD.md`.

**Why:** §2f counts how many sources a page is built on. It does not count which sentences each source supports. A page can list 10 sources in frontmatter while reading like an uncited essay. The Celiac Disease page (1 reference total) is the visible symptom — the underlying pattern is claims floating free of sources across hundreds of pages. Wikipedia's credibility comes from claim-level footnotes, not a list at the bottom. WikiBiome must match.

**How to apply:** When writing or updating a publishable page, cite at the point of the claim — not just in a References section at the bottom. Sentence-level attribution is ideal; paragraph-level is the floor. The ingest cycle includes a self-check before commit: if the page being written has fewer than one inline `[[source-filename]]` per three paragraphs, revise before committing. The maintenance cycle scans existing pages on the same metric and demotes offenders.

### Rule 12 — Adversarial audit as steady state

**Rule:** Every nightly maintenance cycle runs an adversarial read against 5 sampled publishable pages, weighted toward pages with no `audit_passed` date in frontmatter or the oldest dates. The audit persona is a hostile reviewer: where is the citation for this sentence, what claim is made without evidence, what contradiction is glossed over, what would a skeptical domain expert attack first. Findings are written to `wiki/analyses/adversarial-audit-YYYY-MM-DD.md`. Remediation is applied in the same cycle when possible — adding missing citations from the existing source pool, splitting overreaching claims, or demoting the page to `stub: true`. Pages that pass clean audits three cycles in a row carry `audit_passed: YYYY-MM-DD` in frontmatter and are deprioritized in subsequent sampling.

**Why:** Lint catches structural violations — missing frontmatter, broken wikilinks, threshold failures. It does not catch a well-structured page that is factually shallow or argumentatively thin. Adversarial audit is the only process that surfaces "this page looks fine but reads like Wikipedia circa 2005 — uncited generalizations strung together." Without it, the vault accumulates pages that pass lint forever while still failing the reader.

**How to apply:** At the start of every maintenance cycle, sample 5 pages by the weighting above. For each: read as a hostile reviewer, list every claim lacking citation, every contradiction with another vault page, every hand-wave. Write findings to the audit file. If citations exist elsewhere in the vault to fix the gap, apply them and recommit. If the page is fundamentally thin, demote to stub and note why. Never silently pass a page — always write the finding, even if "no issues" is the finding.

### Rule 13 — Auto-discovery of missing scope

**Rule:** Every maintenance cycle scans the vault for conditions, metals, microbes, and concepts that are mentioned in source pages but lack their own entity, signature, or concept page. For each missing target with ≥3 distinct source mentions, create a stub at the appropriate path with `stub: true`, `stub_reason: "auto-discovered from source mention — awaiting content expansion"`, and the minimal valid frontmatter for the type. The system never waits for a human to notice a scope gap; the scope gap creates itself into existence as a tracked stub. Auto-created stubs are logged to `wiki/analyses/auto-discovered-stubs-YYYY-MM-DD.md`.

**Why:** Karen should not have to audit the vault for missing pages. If a source mentions *Fusobacterium nucleatum* across 40 source files and no `wiki/entities/fusobacterium-nucleatum.md` exists, that is a bug the system fixes itself — not a task that waits for Karen to file it. The same holds for conditions named in signatures but missing disease entity pages, metals referenced without an entity page, and concepts mentioned without a concept page.
