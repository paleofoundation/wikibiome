# WikiBiome concept coverage program

Generated deterministically from corpus `7e64b5121603`, catalog `b3d82c6dacee`, and decisions `2e9c853c6200`. This is an editorial queue, not an auto-publishing instruction. Candidate-source counts identify records to inspect; they do not certify that every record directly supports a future page.

## Summary

- 4 create-page candidates
- 202 existing pages with contextual-link debt
- 1 candidates held for evidence
- 0 explicitly accepted jobs
- 16 completed jobs recorded

## Active reviewed batch

_No explicitly accepted work is awaiting promotion._

## Completed jobs

- `backfill_links:ammonia#2026-09-07-core-ammonia-contexts` — 2026-09-07 — targets: `urea`, `urease`, `helicobacter-pylori`, `proteus-mirabilis`, `ureaplasma`
- `backfill_links:caga#2026-09-07-core-caga-contexts` — 2026-09-07 — targets: `hydrogenase`, `gastric-cancer`, `gastric-adenocarcinoma`, `nickel`, `helicobacter-pylori`, `metal-dependent-virulence`, `gastric-ulcer`, `nickel-transporters`, `virulence-factors`
- `backfill_links:catalase#2026-09-07-core-catalase-contexts` — 2026-09-07 — targets: `oxidative-stress`, `streptococcus-mutans`, `metal-dependent-virulence`, `iron`, `aerococcus`
- `backfill_links:interleukin-22#2026-09-07-core-interleukin-22-contexts` — 2026-09-07 — targets: `tryptophan-metabolism`, `necrotizing-enterocolitis`, `indoles`, `ahr`, `mycobiome`, `immune-balance`, `bacteroides-vulgatus`, `tryptophan`, `bacteroides-thetaiotaomicron`, `prevotella-copri`, `th17-treg-balance`, `debaryomyces`, `infant-exposure`, `peptostreptococcus-stomatis`, `ruminococcus-gnavus`, `indole`, `ketone-bodies`
- `backfill_links:nlrp3-inflammasome#2026-09-07-core-nlrp3-contexts` — 2026-09-07 — targets: `dysbiosis`, `cadmium`, `zinc`, `inflammation`, `colorectal-cancer`, `gut-metal-microbiome`, `cardiovascular-disease`, `chronic-kidney-disease`, `neuroinflammation`, `hashimotos-thyroiditis`, `chromium`, `tmao`, `microglia`, `il-1beta`
- `create_page:ammonia` — 2026-09-07 — `wiki/concepts/ammonia.md`
- `create_page:caga` — 2026-09-07 — `wiki/concepts/caga.md`
- `create_page:catalase` — 2026-09-07 — `wiki/concepts/catalase.md`
- `create_page:interleukin-22` — 2026-09-07 — `wiki/concepts/interleukin-22.md`
- `create_page:nlrp3-inflammasome` — 2026-09-07 — `wiki/concepts/nlrp3-inflammasome.md`
- `backfill_links:histidine#2026-09-06-helicobacter-pylori` — 2026-09-06 — targets: `helicobacter-pylori`
- `backfill_links:hydrogenase#2026-09-06-helicobacter-pylori` — 2026-09-06 — targets: `helicobacter-pylori`
- `backfill_links:nickel-transporters#2026-09-06-helicobacter-pylori` — 2026-09-06 — targets: `helicobacter-pylori`
- `backfill_links:tight-junctions#2026-09-06-helicobacter-pylori` — 2026-09-06 — targets: `helicobacter-pylori`
- `backfill_links:urease#2026-09-06-helicobacter-pylori` — 2026-09-06 — targets: `helicobacter-pylori`
- `create_page:urea` — 2026-09-06 — `wiki/concepts/urea.md`

## Create-page queue

| Score | Job | Term | State | Reader demand | Graph gain | Candidate sources |
|---:|---|---|---|---:|---:|---:|
| 76.72 | `create_page:gastritis` | Gastritis | queued | 10 articles | 45.3 | 10 |
| 76.46 | `create_page:inducible-nitric-oxide-synthase` | Inducible nitric oxide synthase | queued | 13 articles | 66.2 | 14 |
| 72.57 | `create_page:histamine` | Histamine | queued | 10 articles | 45.0 | 5 |
| 49.01 | `create_page:urea-cycle` | Urea cycle | queued | 2 articles | 9.5 | 3 |

## Internal-link backfill queue

| Score | Job | Term | State | Reader demand | Graph gain | Candidate sources |
|---:|---|---|---|---:|---:|---:|
| 98.50 | `backfill_links:urease` | Urease | queued | 60 unlinked / 74 mentioning | 261.0 | 30 |
| 95.50 | `backfill_links:tight-junctions` | Tight junctions | queued | 99 unlinked / 102 mentioning | 437.3 | 69 |
| 95.50 | `backfill_links:hydrogenase` | Hydrogenase | queued | 42 unlinked / 52 mentioning | 190.1 | 14 |
| 94.38 | `backfill_links:ammonia` | Ammonia | queued | 32 unlinked / 37 mentioning | 142.3 | 18 |
| 92.88 | `backfill_links:histidine` | Histidine | queued | 26 unlinked / 29 mentioning | 116.7 | 29 |
| 92.50 | `backfill_links:dysbiosis` | Dysbiosis | queued | 244 unlinked / 327 mentioning | 879.6 | 915 |
| 92.50 | `backfill_links:inflammation` | Metal-Driven Inflammation | queued | 225 unlinked / 279 mentioning | 828.5 | 607 |
| 92.50 | `backfill_links:gut-microbiome` | Gut Microbiome | queued | 216 unlinked / 221 mentioning | 740.8 | 665 |
| 92.50 | `backfill_links:butyrate` | Butyrate | queued | 171 unlinked / 204 mentioning | 638.8 | 306 |
| 92.50 | `backfill_links:oxidative-stress` | Oxidative Stress | queued | 124 unlinked / 184 mentioning | 485.4 | 373 |
| 92.50 | `backfill_links:heavy-metals` | Heavy Metals | queued | 108 unlinked / 111 mentioning | 462.9 | 235 |
| 92.50 | `backfill_links:metabolites` | Microbial Metabolites | queued | 111 unlinked / 112 mentioning | 434.6 | 364 |
| 92.50 | `backfill_links:metallomics` | Metallomics | queued | 77 unlinked / 87 mentioning | 332.2 | 80 |
| 92.50 | `backfill_links:neuroinflammation` | Neuroinflammation | queued | 73 unlinked / 94 mentioning | 326.2 | 196 |
| 92.50 | `backfill_links:tlr4` | TLR4 | queued | 68 unlinked / 76 mentioning | 301.2 | 87 |
| 92.50 | `backfill_links:tryptophan` | Tryptophan | queued | 70 unlinked / 76 mentioning | 291.5 | 202 |
| 92.50 | `backfill_links:biofilm` | Biofilm | queued | 79 unlinked / 103 mentioning | 287.4 | 77 |
| 92.50 | `backfill_links:acetate` | Acetate | queued | 71 unlinked / 71 mentioning | 284.0 | 105 |
| 92.50 | `backfill_links:glutathione` | Glutathione (GSH) | queued | 51 unlinked / 65 mentioning | 240.7 | 84 |
| 92.50 | `backfill_links:systemic-inflammation` | Systemic Inflammation | queued | 60 unlinked / 65 mentioning | 233.9 | 133 |
| 92.50 | `backfill_links:biomarkers` | Biomarkers | queued | 50 unlinked / 64 mentioning | 232.0 | 278 |
| 92.50 | `backfill_links:siderophores` | Siderophores | queued | 58 unlinked / 71 mentioning | 223.2 | 54 |
| 92.50 | `backfill_links:neurodegeneration` | Neurodegeneration and Metals | queued | 52 unlinked / 56 mentioning | 222.8 | 60 |
| 92.50 | `backfill_links:probiotics` | Probiotics | queued | 56 unlinked / 76 mentioning | 210.0 | 417 |
| 92.50 | `backfill_links:nutritional-immunity` | Nutritional Immunity (Metal Sequestration) | queued | 49 unlinked / 174 mentioning | 209.3 | 168 |
| 92.50 | `backfill_links:mendelian-randomization` | Mendelian Randomization | queued | 56 unlinked / 59 mentioning | 209.1 | 106 |
| 92.50 | `backfill_links:virulence-factors` | Virulence Factors | queued | 47 unlinked / 50 mentioning | 195.7 | 29 |
| 92.50 | `backfill_links:bile-acid-metabolism` | Bile Acid Metabolism | queued | 48 unlinked / 67 mentioning | 178.8 | 104 |
| 92.50 | `backfill_links:fenton-chemistry` | Fenton Chemistry | queued | 39 unlinked / 50 mentioning | 178.7 | 48 |
| 92.50 | `backfill_links:environmental-metal-exposure` | Environmental Metal Exposure | queued | 40 unlinked / 58 mentioning | 164.7 | 41 |

## Evidence hold

| Score | Job | Term | State | Reader demand | Graph gain | Candidate sources |
|---:|---|---|---|---:|---:|---:|
| 54.17 | `evidence_hold:bicarbonate` | Bicarbonate | queued | 11 articles | 49.8 | 2 |

## Publication gate

- Resolve aliases, parent-page overlap, and duplicate identities before drafting.
- Validate at least three independent sources as directly supporting the page, not merely mentioning the term.
- Cite every material claim inline and preserve the WikiBiome/Cureva boundary.
- Backfill only the first contextually meaningful occurrence in a reviewed article.
- Require clean content, editorial, routing, and rendered-page checks before publication.
