...
5. After a substantive batch, validate locally once: `node scripts/build-content.cjs && npx vite build && node scripts/generate-static.cjs`. Prepare the changes for one release PR. WB-COST-KILL-1 forbids preview and production deployment from this cycle; publish the merged release batch once through the owning Sites checkout. Do not push directly to main, retry a deployment, or write a deploy command to the clipboard.
